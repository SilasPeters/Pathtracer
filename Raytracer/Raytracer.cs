...
namespace Template
{
	static class Raytracer
	{
		...
		public const float AmbientLightLevel = 0.7f;
		...
		{
			Material Blue     = new Material(new Vector3(0, 0, 1),           Vector3.One / 2, Vector3.One / 2);
			Material Greenish = new Material(new Vector3(80, 189, 58) / 255, Vector3.One / 2, Vector3.One / 2);
			Material purple = new Material(new Vector3(1, 0, 1),           Vector3.One / 2, Vector3.One / 2);
			Material mir      = new Material(new Vector3(1, 1, 1),           Vector3.One / 2, Vector3.One, "Mirror");

			Scene.AddObject(new Sphere(new Vector3(3,  2,    0), 1.3f, Blue));
			Scene.AddObject(new Sphere(new Vector3(0,  0.5f, 0), 1f,   mir));
			Scene.AddObject(new Sphere(new Vector3(-2, 1f,   0), 1f,   purple));
			Scene.AddObject(new Sphere(new Vector3(-1,  3f,   3), 2f,   Greenish));

			Scene.AddObject(new Plane(new Vector3(0, -1f, 0), new Vector3(0, -1, 0), Greenish));

			Scene.AddLight(new LightSource(new Vector3(-3,  3, 0f), new Vector3(1, 1, 1)*3));
